...
export const ROIInputSection = ({
  ...
  >
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-8 border-b border-aetox-border">
      <span className="text-xs md:text-sm font-bold text-aetox-text-muted">{dict.inputs.parametersLabel}</span>
      <CurrencySwitcher />
    </div>

    <div className="space-y-4">
      <p className="text-[11px] md:text-xs font-bold text-aetox-accent">{dict.inputs.presetsLabel}</p>
      <div className="grid grid-cols-3 gap-3">
        {['startup', 'sme', 'enterprise'].map((id) => (
          <button
            key={id}
            onClick={() => applyPreset(id as any)}
            className="py-3 md:py-4 rounded-xl border border-aetox-border bg-aetox-bg text-[11px] md:text-xs font-bold text-aetox-text-soft hover:border-aetox-accent hover:text-aetox-text-main hover:bg-aetox-accent/10 transition-all"
          >
    ...
    <div className="space-y-8">
      <div className="space-y-4">
        <div className="flex justify-between items-center gap-4">
          <label className="text-xs md:text-sm font-bold text-aetox-text-soft">{dict.inputs.staffCount}</label>
          <span className="text-aetox-text-main font-bold text-xl md:text-2xl whitespace-nowrap">{staffCount} {dict.common.units.people}</span>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center gap-4">
          <label className="text-xs md:text-sm font-bold text-aetox-text-soft">{dict.inputs.avgSalary}</label>
          <span className="text-aetox-text-main font-bold text-xl md:text-2xl whitespace-nowrap">{formatCurrency(avgSalary)}</span>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center gap-4">
          <label className="text-xs md:text-sm font-bold text-aetox-text-soft">{dict.inputs.hoursSpent}</label>
          <span className="text-aetox-text-main font-bold text-xl md:text-2xl whitespace-nowrap">{hoursSpent} {dict.common.units.hoursPerDay}</span>
        </div>
