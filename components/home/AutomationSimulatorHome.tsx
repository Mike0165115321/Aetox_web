...
export default function AutomationSimulatorHome({ dict }: { dict: any }) {
  ...
  return (
    <div className="w-full bg-[#080B14]/40 backdrop-blur-xl rounded-[40px] border border-white/10 overflow-hidden shadow-2xl">
      {/* Header */}
      <div className="px-8 py-6 border-b border-white/5 bg-white/[0.02] flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 uppercase tracking-widest mt-1">{dict.subTitle}</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row">
        {/* Left: Inputs */}
        <div className="lg:w-[40%] p-8 border-b lg:border-b-0 lg:border-r border-white/5 space-y-10">
          <section className="space-y-4">
            <h4 className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.2em] border-l-4 border-[#06B6D4] pl-4">
              {dict.workloadTitle}
            </h4>
              ...
              {(Object.keys(workloadConfig) as Complexity[]).map((lvl) => {
                ...
                return (
                  <button 
                    ...
                    className={`flex items-center gap-4 px-5 py-4 rounded-2xl text-left transition-all border ${active ? 'bg-[#06B6D4]/10 border-[#06B6D4]/40 text-white' : 'bg-white/[0.02] border-transparent text-gray-500 hover:bg-white/[0.05]'}`}
                  >
        ...
        {/* Right: Results */}
        <div className="lg:w-[60%] p-8 bg-black/20 space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            />
            <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.03] flex flex-col justify-between">
              <p className="text-[11px] text-gray-500 font-bold mb-3 tracking-wide uppercase">{dict.kpis.payback}</p>
              <div className="space-y-1">
                <p className="text-4xl font-bold text-emerald-400">{calc.paybackMonths < 100 ? calc.paybackMonths.toFixed(1) : '—'}</p>
                <p className="text-sm text-gray-500 font-bold uppercase">{dict.kpis.paybackUnit}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 rounded-3xl bg-rose-500/5 border border-rose-500/10 space-y-4">
              <p className="text-xs font-bold text-rose-400 uppercase tracking-widest">{dict.comparison.manual}</p>
              <div className="space-y-3">
                <div className="flex justify-between text-sm"><span className="text-gray-500">{dict.comparison.totalHours}</span><span className="text-white font-bold">{calc.manualHours.toFixed(0)} ชม.</span></div>
                <div className="flex justify-between text-sm border-t border-white/5 pt-3"><span className="text-gray-500">{dict.comparison.totalCost}</span><span className="text-rose-400 font-bold">{formatMoney(calc.totalBefore)}</span></div>
              </div>
            </div>
            <div className="p-6 rounded-3xl bg-emerald-500/5 border border-emerald-500/10 space-y-4">
              <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest">{dict.comparison.bot}</p>
              <div className="space-y-3">
                <div className="flex justify-between text-sm"><span className="text-gray-500">{dict.comparison.invest}</span><span className="text-white font-bold">{formatMoney(botPrice)}</span></div>
                <div className="flex justify-between text-sm border-t border-white/5 pt-3"><span className="text-gray-500">{dict.comparison.maint}</span><span className="text-emerald-400 font-bold">{formatMoney(maintCost)}/ด.</span></div>
              </div>
            </div>
          </div>

          {/* Benchmark */}
          <div className="p-8 rounded-3xl bg-white/[0.03] border border-white/5 space-y-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-6 opacity-5"><Zap size={80} /></div>
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-white/5 pb-4">{dict.benchmark.title}</h4>
            <div className="space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between items-end text-sm"><span className="font-bold text-white">{dict.benchmark.botLabel}</span><span className="font-bold text-emerald-400">{calc.botHours.toFixed(1)} {dict.benchmark.unitTime}</span></div>
                <div className="w-full h-3 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)] transition-all duration-1000" style={{ width: `${calc.botBarPct}%` }} /></div>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between items-end text-sm"><span className="font-bold text-gray-500">{dict.benchmark.manualLabel}</span><span className="font-bold text-rose-400">{calc.manualHours.toFixed(1)} {dict.benchmark.unitTime}</span></div>
                <div className="w-full h-3 bg-white/5 rounded-full overflow-hidden"><div className="h-full w-full bg-rose-500/50" /></div>
              </div>
            </div>
          </div>
        </div>
