    ...
    <div className="flex flex-col h-full">
      ...
      {features && (
        <ul className="space-y-4 mb-10 flex-grow">
          {features.map((feature, i) => (
            <li key={i} className="flex items-start gap-3 text-fluid-sm font-bold text-aetox-text-soft leading-tight">
      ...
      {relatedProjects && relatedProjects.length > 0 && (
        <div className="mt-auto pt-8 border-t border-aetox-border/50 w-full">
          <p className="text-fluid-label text-aetox-text-muted mb-4 font-bold">{deploymentLabel || 'Enterprise Deployment'}</p>
