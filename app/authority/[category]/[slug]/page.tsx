...
export default async function ProjectDetailPage({ params }: { params: { category: string, slug: string } }) {
  ...
  const project = projectsContent.items.find(item => item.category === category && item.slug === slug);
  ...
  return (
    <main className="min-h-screen bg-ultra-dark text-white selection:bg-cyber-blue/30 selection:text-white relative">
      ...
      <article className="container mx-auto pt-40 pb-32 relative z-10">
        ...
        <Link 
          href="/authority" 
          className="inline-flex items-center gap-2 text-gray-500 hover:text-cyber-blue transition-colors mb-12 group"
        >
          ...
          <div className="space-y-10">
            ...
            <div className="flex flex-wrap gap-3">
              {project.tags.map(tag => (
                <span key={tag} className="px-4 py-2 bg-white/5 rounded-xl border border-white/10 text-xs font-semibold text-gray-300">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-6">
              {project.githubUrl && (
                <a 
                  ...
                  target="_blank" 
                  className="flex items-center gap-3 px-8 py-4 bg-white text-black font-black uppercase tracking-widest text-sm rounded-2xl hover:bg-cyber-blue hover:text-white transition-all transform active:scale-95 shadow-cyber-glow"
                >
              ...
              {project.liveUrl && (
                <a 
                  ...
                  target="_blank" 
                  className="flex items-center gap-3 px-8 py-4 bg-white/5 border border-white/10 text-white font-black uppercase tracking-widest text-sm rounded-2xl hover:bg-white/10 transition-all transform active:scale-95"
                >
          ...
          <div className="lg:pl-20">
            <div className="glass-card p-10 rounded-3xl border-white/5 space-y-8">
              ...
              <div className="space-y-6 pt-4">
                ...
                <div>
                  <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-1">Deployment Scope</p>
                  <p className="text-lg font-bold text-white">Full Stack Engineering</p>
                </div>
              </div>
            </div>
          </div>
