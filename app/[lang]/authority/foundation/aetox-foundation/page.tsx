...
export default async function AetoxFoundationPage() {
  const navDict = await getDictionary('th', 'navigation');
  const project = projectsContent.items.find(item => item.slug === 'aetox-foundation');
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
          <div className="space-y-10">
            ...
            <div className="flex flex-wrap items-center gap-4 pt-6">
              {project.liveUrl && (
                <a href={project.liveUrl} target="_blank" className="flex items-center gap-3 px-8 py-4 bg-white/5 border border-white/10 text-white font-black uppercase tracking-widest text-sm rounded-2xl hover:bg-white/10 transition-all transform active:scale-95 shadow-cyber-glow">
