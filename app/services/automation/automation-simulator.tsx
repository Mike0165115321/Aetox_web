  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-8 pb-20">
      ...
      <div className="bg-[#0A0F1C] text-white rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden">
        {/* HEADER */}
        <div className="px-6 sm:px-8 py-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center bg-white/[0.02] gap-4">
          ...
          <div className="flex items-center gap-3">
             <div className="bg-black/40 p-1 rounded-xl border border-white/10 flex items-center gap-1">
               <button onClick={() => setCurrency('THB')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${currency === 'THB' ? 'bg-[#06B6D4] text-white' : 'text-gray-500'}`}>THB</button>
               <button onClick={() => setCurrency('USD')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${currency === 'USD' ? 'bg-[#06B6D4] text-white' : 'text-gray-500'}`}>USD</button>
             </div>
             <button onClick={() => updateComplexity('medium')} className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-all"><RotateCcw size={16} /></button>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row">
                  ...
                  return (
                    <button key={lvl} onClick={() => updateComplexity(lvl)}
                      className={`flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-bold transition-all duration-300 border text-left active:scale-95 ${
                        active ? 'bg-white/10 text-white border-[#06B6D4]/40 shadow-[0_0_20px_rgba(6,182,212,0.05)]' : 'text-gray-400 border-transparent bg-white/[0.01]'}`}>
          ...
          <div className="lg:w-[65%] p-6 sm:p-8 space-y-8 bg-black/5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <KpiCard label="ประหยัดได้ / เดือน" valTHB={calc.monthlySaving} accent color="text-emerald-400" detail="ยอดเงินประหยัดรายเดือนสุทธิหลังจากหักลบค่าใช้จ่ายระบบแล้ว" />
